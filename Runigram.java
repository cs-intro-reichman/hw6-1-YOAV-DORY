...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for(int i = 0; i < image.length; i++){
			for(int j = 0; j < image[0].length; j++){
				newImage[i][image[0].length-1 - j] = image[i][j];
			}
		}
		return newImage;
	}
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for(int i = 0; i < image.length; i++){
			for(int j = 0; j < image[0].length; j++){
				newImage[image[0].length-1-j][i] = image[j][i];
			}
		}
		return newImage;
	}
	...
	private static Color luminance(Color pixel) {
		...
		Color lumiColor = new Color(lum , lum , lum);
		return lumiColor;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for(int i = 0; i < image.length; i++){
			for(int j = 0; j < image[0].length; j++){
				newImage[i * (width0/width)][j * (height0/height)] = image[i][j];
			}
		}
		return newImage;
	}
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		for(int i = 0; i <= n; i++){
			Color[][] stepImage = blend(source, scaledTarget,(n - i)/n);
			display(stepImage);
			StdDraw.pause(500);
		}
	}
